...
namespace cAlgo
{
    ...
    public class OrderFlowTicksV20 : Indicator
    {
        ...
        public class IndicatorParams
        {
            ...
            public bool ShowMinMaxDelta { get; set; }
        }
        ...
        protected override void Initialize()
        {
            ...
            IndicatorParams DefaultParams = new()
            {
                ...
                ShowMinMaxDelta = ShowMinMaxDeltaInput
            };
                ...
                */
                ...
                if (VolumeModeInput == VolumeModeData.Normal)
                {
                    ...
                    if (currentTimeframe.Contains("Renko") || currentTimeframe.Contains("Range"))
                        ...
                        bool condition = Bars.OpenTimes[iStart].DayOfWeek == DayOfWeek.Friday && Bars.OpenTimes[iStart + 1].DayOfWeek == DayOfWeek.Sunday;
                        if (condition)
                            volHist = Chart.DrawRectangle($"{iStart}_{i}", Bars.OpenTimes[iStart], lowerSegment, Bars[iStart].OpenTime.AddMilliseconds(dynLength), upperSegment, dynColor);
                        else
    ...
    public class ParamsPanel : CustomControl
    {
        ...
        private const string ShowMinMaxDelta_InputKey = "ShowMinMaxDeltaKey";
        // Buy vs Sell
        ...
        private ControlBase ShowMinMaxDeltaPanel;
        ...
        private StackPanel CreateContentPanel()
        {
            ...
            ShowMinMaxDeltaPanel = CreateCheckboxWithLabel("Show Min/Max?", FirstParams.ShowMinMaxDelta, ShowMinMaxDelta_InputKey);
            ShowMinMaxDeltaPanel.IsVisible = true;
            ...
            grid.AddChild(ShowMinMaxDeltaPanel, 2, 2, 2, 4);
        ...
        private void NextModeEvent(ButtonClickEventArgs obj)
        {
            if (Outside.VolumeModeInput == VolumeModeData.Normal)
            {
                ...
                ShowMinMaxDeltaPanel.IsVisible = false;
            }
            else if (Outside.VolumeModeInput == VolumeModeData.Buy_Sell)
            {
                ...
                ShowMinMaxDeltaPanel.IsVisible = Outside.VolumeViewInput == VolumeViewData.Profile;
        ...
        private void ChangeParams(IndicatorParams indicatorParams)
        {
            foreach (var key in checkBoxMap.Keys)
            {
                switch (key)
                {
                    ...
                    case "ShowMinMaxDeltaKey": checkBoxMap[key].IsChecked = indicatorParams.ShowMinMaxDelta; break;
                }
            }
                ...
                switch (key)
                {
                    ...
                    case "ProfileOrDividedKey": {
                        ...
                        if (selected == VolumeViewData.Divided.ToString()) {
                            ...
                            ShowMinMaxDeltaPanel.IsVisible = !EmptyBuySellPanel.IsVisible;
                            OnlyLarguestPanel.IsVisible = false;
                        }
                        break;
                    }
        ...
        private void CheckBoxClickEvent(CheckBoxEventArgs obj)
        {
            ...
            value = GetValueFromCheckbox(ShowMinMaxDelta_InputKey, true);
            if (value != Outside.ShowMinMaxDeltaInput) {
                Outside.ShowMinMaxDeltaInput = value;
                RecalculateOutsideWithMsg();
            }
        }
