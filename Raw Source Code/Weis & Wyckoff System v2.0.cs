...
-Fix indicator's objects being randomly removed right after switching Panel settings.
    - To keep the update “static,” the first attempt will remove some objects,
    while subsequent attempts will not remove them, only change.
-Fix wrongly implementation of Right comparison mark.
    ...
    public class WeisWyckoffSystemV20 : Indicator
    {
        ...
        */
            ...
            void WW_Analysis(double cumlVolume, bool isUp)
            {
                ...
                if (isUp)
                {
                    ...
                    if (ShowMarksInput == ShowMarksData.Left)
                        ...
                        rightMark = cumlVolume > prevWaveVol_Down ? "🡩" : "🡫";
                    else if (ShowMarksInput == ShowMarksData.Both)
                    {
                        leftMark = cumlVolume > prevWaveVol_Up ? "⮝" : "⮟";
                        rightMark = cumlVolume > prevWaveVol_Down ? "" : leftMark == "⮟" ? "" : "🡫";
                    }
                ...
                else
                {
                    ...
                    if (ShowMarksInput == ShowMarksData.Left)
                        ...
                        rightMark = cumlVolume > prevWaveVol_Up ? "🡫" : "🡩";
                    else if (ShowMarksInput == ShowMarksData.Both)
                    {
                        leftMark = cumlVolume > prevWaveVol_Down ? "⮟" : "⮝";
                        rightMark = cumlVolume > prevWaveVol_Up ? "" : leftMark == "⮝" ? "" : "🡩";
                    }
            ...
            void EvsR_Analysis(double cumlVolPrice, bool endWave, bool isBullish)
            {
                ...
                if (isBullish)
                {
                    ...
                    if (ShowMarksInput == ShowMarksData.Left)
                        ...
                        rightMark = cumlVolPrice > prevVolPrice_Down ? "🡩" : "🡫";
                    else if (ShowMarksInput == ShowMarksData.Both)
                    {
                        leftMark = cumlVolPrice > prevVolPrice_Up ? "⮝" : "⮟";
                        rightMark = cumlVolPrice > prevVolPrice_Down ? "" : leftMark == "⮟" ? "" : "🡫";
                    }
                ...
                else
                {
                    ...
                    if (ShowMarksInput == ShowMarksData.Left)
                        ...
                        rightMark = cumlVolPrice > prevVolPrice_Up ? "🡫" : "🡩";
                    else if (ShowMarksInput == ShowMarksData.Both)
                    {
                        leftMark = cumlVolPrice > prevVolPrice_Down ? "⮟" : "⮝";
                        rightMark = cumlVolPrice > prevVolPrice_Up ? "" : leftMark == "⮝" ? "" : "🡩";
                    }
        ...
        public void ClearAndRecalculate()
        {
            ...
            // With 2 Chart.Re[...] one here (async), one before (sync) gives a nice impression of static update.
            // If the issue occurs again in newer versions, remove this.
